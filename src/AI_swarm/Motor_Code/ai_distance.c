...
Distanzberechnung �ber eigenen Thread?
	-- workerSchedule habe ich mich schon drum gek�mmert - Nicolai
...
{ 
	...
	Master verteilt vollst�ndige Tabelle
}
...
void UpdateDistanceTable(st_distances_t * distanceTable)
{
	...
	if (my_ai_role == MASTER)
	{
		...
		distribute_distances(); //versendet vollst�ndige Distanztabelle
	}
...
//versendet vollst�ndige Distanztabelle
